  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-10">
      ...
      <div className="mt-8 rounded-2xl border border-slate-700 bg-slate-900 p-5">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">{game.players.map(player => <div key={player.id} className="flex justify-between rounded-lg border border-slate-700 px-3 py-3"><span>{player.name}</span><span className="font-mono text-amber-300">${player.stack}</span></div>)}</div>
        <p className="mt-5 text-xs text-slate-500">Hole cards are hidden while the hand is in progress.</p>
      </div>
    </main>
  )
}
